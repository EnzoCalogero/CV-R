#-------------------------------------------------------------------------------
# Name:        Import wata written per hour
#
...
#This function estimates teh amount of data written each hour...

def readDB_data_Written(SPolicy):
     matriceDataHrJob= [[1 for j in range(25)] for i in range(7)]
 #    matriceTime= [[1 for j in range(24)] for i in range(7)]
     for day in range(0,7):
         sql = "SELECT  startdateDay,startdateMese,startdateAnno FROM Jobs where startSTANDART=="+ str(24* (day+inizialeDay)+6)
         timestamp=cursor.execute(sql)
         for row in timestamp:
             matriceDataHrJob[day][24]=row
         for hour in range(24):
 #           matriceTime[day][hour]=day*24+hour
            if(str(SPolicy) !="All"):
                sql = "SELECT sum (numbytescomp) FROM Jobs where data_sp= \'" + str(SPolicy) +"\' and startSTANDART<="+ str(24* (day+inizialeDay)+hour+timezone) + " and endSTANDART>="
                sql =sql+ str( 24*(day+inizialeDay)+hour+timezone)+ " and startdateMese=" + str((inizialeMonth))
            else:
                sql = "SELECT sum (numbytescomp) FROM Jobs where startSTANDART<="+ str(24* (day+inizialeDay)+hour+timezone) + " and endSTANDART>="
                sql =sql+ str( 24*(day+inizialeDay)+hour+timezone)+ " and startdateMese=" + str((inizialeMonth))
    ...
    #   Popolate the time
            for row in ai:
                a=row[0]
                #print(a)
                matriceDataHrJob[day][hour]=a
     print("matrice")
     print("############")
     print(matriceDataHrJob)
     return(matriceDataHrJob)


#This collect the amount of data that is requested to be backup

def readDB_data(SPolicy):
     matriceDataJob= [[1 for j in range(25)] for i in range(7)]
 #    matriceTime= [[1 for j in range(24)] for i in range(7)]
     for day in range(0,7):
         #Populate the time....
         sql = "SELECT  startdateDay,startdateMese,startdateAnno FROM Jobs where startSTANDART=="+ str(24* (day+inizialeDay)+6+timezone)
         timestamp=cursor.execute(sql)
         for row in timestamp:
             matriceDataJob[day][24]=row
         for hour in range(24):
 #           matriceTime[day][hour]=day*24+hour
            if(str(SPolicy) !="All"):
                sql = "SELECT sum (numbytescomp) FROM Jobs where data_sp= \'" + str(SPolicy) +"\' and startSTANDART<="+ str(24* (day+inizialeDay)+hour+timezone) + " and endSTANDART>="
          ...
          #  print (sql)
            ...
            for row in ai:
                a=row[0]
                #print(a)
                matriceDataJob[day][hour]=a
     print("matrice")
     print("############")
     print(matriceDataJob)
     return(matriceDataJob)
...
#    path.replace(" ", "k")
 ...
 #   stop()
    ...
    for d in range(0,7):
          ...
          for h in range(0,25):
               if(h>0) :fout.write(",")
               fout.write(str(matriceDataJob[d][h]))
    fout.close()

def writeSP_data_Hr(Ppath,CS, DAY, month, year,StoragePolicy):
    SP=str(StoragePolicy)
    SP=SP.replace(" ", "")
    SP=SP.replace("/", "_")
    path=Ppath +"\\"+ CS +"\\"+ str(DAY) +"_"+ str(month) +"_"+ str(year) +"\\"+ SP + "_data.csv"
#    path+str(path)
#    path.replace(" ", "k")
    print(path)
 #   stop()
    ...
    for d in range(0,7):
          ...
          for h in range(0,25):
               if(h>0) :fout.write(",")
               fout.write(str(matriceDataHrJob[d][h]))
    fout.close()
...
def csv_reader(file_obj):
    ...
    cursor.execute(''' CREATE TABLE Jobs(
                       ...
                       troughput DOUBLE,numobjects INTEGER)
                    ''')
    db.commit()
    ...
    for row in reader:
      # data written in each hour....trasform sec in hours....
      troughput=float(row["numbytescomp"])/(math.ceil(float(row["durationunixsec"])+1)/3600)
...
#    matriceNumJob=readDB(SPolicy)
    ...
    matriceDataHrJob=readDB_data_Written(SPolicy)

    print("matrice data")
    print(matriceDataHrJob)
    #stop()
    matriceDataJob=readDB_data(SPolicy)
    writeSP_data_Hr(Ppath,CS,DAY,month,year,SPolicy)

...
CriticalSizeTrend<-function(sidb=0,Mo=10,file='C:/temp/lon3/temp.csv'){
  ...
  ######
  View(AFID)
 # RankALL<-aggregate(AFID~Date2+DDBID+Host+timeHs, max,data=AFID)
#  RankALL$CS<-as.character(substr(RankALL$DDBID,1,5))
 # View(RankALL)
  ...
  m1 <- ggplot(AFID, aes(x = Date2,y=AFID))+  geom_point(aes(colour = DDBID))  + facet_grid(CS~.)+ ggtitle("AFID TREND OVER 6pm")
  # Temp Out
  #multiplot(m1, cols=1)
  ggplotly(m1)
...
#  View(RankALL)
  ...
  #m <- ggplot(AFID, aes(x = Date2,y=AFID))+  geom_point(aes(colour = DDBID))  + facet_grid(CS~.)+ ggtitle("AFID TREND OVER TIME2")
  # Temp Out
  #multiplot(m, cols=1)
  #ggplotly(m) 
  ...
  # mt <- ggplot(tempRank, aes(x = Date2,y=AFID))+  geom_point() +geom_line(aes(colour = DDBID)) + ggtitle(cs)
   # Temp Out
#   multiplot(mt, cols=1)

...
SealRead<-function(file='C:/Users/enzo7311/Desktop/Dati/a.csv'){
  ...
  p1<-ggplot(SEALDDB,aes(day )) + geom_point(aes(y=Number.of.Secondary.Blocks)) + geom_point(aes(y=Number.of.Unique.Blocks))
  p2<-ggplot(newDDB,aes(day )) + geom_point(aes(y=SecondaryEntries)) + geom_point(aes(y=PrimaryEntries))
  ...
  p3<-ggplot(Final,aes(Group.1 ))+geom_abline(intercept =maxvalue,slope=0 ) +geom_abline(intercept =2*maxvalue,slope=0 )+geom_point(aes(y=Number.of.Unique.Blocks))+geom_point(aes(y=PrimaryEntries))+ geom_point(aes(y=SOMMAPrim)) + geom_point(aes(y=SOMMASEC))
